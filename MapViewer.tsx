...
export class MapViewer extends Component<MapProps, MapState> {
  ...
  renderEndPoints = (): Array<JSX.Element> => {
    ...
    if (this.state.nearby !== undefined) {
      this.state.nearby.forEach((f, i) => this.doAddNearbyClick(f, i, elems));
    }
  
    return elems;
  };

  doAddNearbyClick = (f: Nearby, i: number, elems: Array<JSX.Element>): void => {
    elems.push(
      <circle
        cx={f.loc.x}
        cy={f.loc.y}
  ...
  renderLegendItems = (): Array<JSX.Element> => {
    ...
    if (this.state.nearby) {
      this.state.nearby.forEach((f, i) => this.doAddLegendItemClick(f, i, items));
    }

    return items;
  };

  doAddLegendItemClick = (friend: Nearby, idx: number, items: Array<JSX.Element>): void => {
    const color = FRIEND_COLORS[idx % FRIEND_COLORS.length];
    items.push(makeLegendItem(color, friend.friend, `friend-${idx}`));
  };
  ...
  doShortestPathJson = (data: unknown): void => {
    ...
    if (data.found) {
      ...
      const nearbyPoints = data.nearby ? parseNearbyList(data.nearby) : [];
