...
describe('routes', function() {
  ...
  it('data_friends', function() {
    ...
    const kevinFriends = ["Alice"];

    const req1 = httpMocks.createRequest({
        method: 'POST',
        url: '/api/setData',
        body: {user: "Kevin", schedule: kevinSchedule, friends: kevinFriends}
    });
    const res1 = httpMocks.createResponse();
    ...
    getUserData(req2, res2);
    assert.deepStrictEqual(res2._getStatusCode(), 200);
    assert.deepStrictEqual(res2._getData(), {schedule: kevinSchedule, friends: ["Alice"]});

    const aliceSchedule = [
        {hour: "9:30", location: "HUB", desc: "Coffee"},
        {hour: "10:30", location: "CS2", desc: "CSE 142"},
        {hour: "11:30", location: "MLR", desc: "Break"},
    ];
    const aliceFriends = ["Kevin"];

    const req3 = httpMocks.createRequest({
        method: 'POST',
        url: '/api/setData',
        body: {user: "Alice", schedule: aliceSchedule, friends: aliceFriends}
    });
    const res3 = httpMocks.createResponse();
    setUserData(req3, res3);
    ...
    getUserData(req4, res4);
    assert.deepStrictEqual(res4._getStatusCode(), 200);
    assert.deepStrictEqual(res4._getData(), {schedule: aliceSchedule, friends: ["Kevin"]});
  })
  ...
  it('getShortestPath', function() {
    ...
    const data = res9._getData();
    assert.deepStrictEqual(data.found, true);
    assert.deepStrictEqual(data.path.length > 0, true);

    assert.deepStrictEqual(data.nearby.length, 1);
    assert.deepStrictEqual(data.nearby[0].friend, "Alice");
    assert.deepStrictEqual(typeof data.nearby[0].dist, "number");
    assert.deepStrictEqual(typeof data.nearby[0].loc, "object");
